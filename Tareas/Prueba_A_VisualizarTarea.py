...
try:
    ...
    # Ir a la página del listado de tareas
    driver.get("http://localhost/proyectoEscuela/admin/tareas/")

    time.sleep(10)

    # Verificar que la tabla de tareas se haya cargado correctamente
    filas = driver.find_elements(By.XPATH, "//table//tbody//tr")

    if len(filas) > 0:
        print(f"Prueba exitosa: se encontraron {len(filas)} tareas en la tabla. :D")
    else:
